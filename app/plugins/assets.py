...
async def checkpoint(addresses, eng):
    # addresses
    ...
    df_addresses.to_sql(f'checkpoint_assets', eng, if_exists='replace')
    ...
    sql = f'''
        insert into assets (address, token_id, amount, box_id, height)
            ...
            from checkpoint_assets c
                left join addresses_assets a on a.address = c.address::text
...
async def process(last_height, use_checkpoint = False, boxes_tablename:str = 'boxes', box_override=''):
    ...
    ###
    ### 1. Remove spent boxes from assets, keys
    ...
    with eng.begin() as con:
        ...
        sql = text(f'''
            ...
            delete from addresses_assets t
            using spent s
    ...
    for r in range(last_r-1, box_count, CHECKPOINT_INTERVAL):
        ...
        try:
            ...
            for address, box_id, assets, value, height in [[u[2]['ergoTree'], u[2]['boxId'], u[2]['assets'], u[2]['value'], u[1]] for u in utxo if u[0] == 200]:
