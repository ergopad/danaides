...
async def daoMembership(addresses: Address):
    try:
        # TODO: get DAOs from config table
        ALL_DAOs = {
            'paideia': dotdict({
        ...
        membership = []
        for tkn, dao in ALL_DAOs.items():
            logger.debug(f'checking membership for: {tkn}')
            tid = TokenInventoryDAO(addresses=addresses.addresses, token_id=dao.token_id, stake_tree=dao.stake_tree, vest_tree=dao.vest_tree, proxy_address=dao.proxy_address)
